...
public class Resultadoactividad2 extends AppCompatActivity {
        ListView  listatodos;
  ...
  Ipconexion ipconexion;
   ...
   String diaselecion ="";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_resultadoactividad2);
    ...
    public String buscarestudiante() {
        ...
        String respuesta = "";
        try {
            URL url = new URL("http://"+ipconexion.getIp()+"/dbandroid/WS/resultadoactividad2.php");
    ...
    public void listarestidiandte (String respuesta){
        ...
        try{
            ...
            for (int i = 0;i<jsonarreglo.length();i++){
                ...
                u.setDiasemana(jsonarreglo.getJSONObject(i).getString("diasemana"));
    ...
    private void obtenerlista() {
        ...
        if (listausuario.size() >= 1){
            for (int i =0; i<listausuario.size();i++){
                listainformacion.add(" Documento : "+listausuario.get(i).documento + "  Nombre : "+listausuario.get(i).nombre+"  estado actividad : "+listausuario.get(i).estadoactividad+"  dia : "+listausuario.get(i).diasemana+"  fecha :  "+listausuario.get(i).fecha);
    ...
    public String buscarestudoc(String doc) {
        ...
        String respuesta = "";
        try {
            URL url = new URL("http://"+ipconexion.getIp()+"/dbandroid/WS/resultadoactividad2documento.php");
            connction = (HttpURLConnection) url.openConnection();
    ...
    public String buscarestudocdia(String doc,String dia) {
        ...
        ipconexion = new Ipconexion();
        String parametros ="documento="+doc+"&diasemana="+dia+"";

        HttpURLConnection connction = null;
        String respuesta = "";
        try {
            URL url = new URL("http://"+ipconexion.getIp()+"/dbandroid/WS/resultadoactividad2documentodia.php");
            connction = (HttpURLConnection) url.openConnection();
            connction.setRequestMethod("POST");
            connction.setRequestProperty("Content-Length",""+Integer.toString(parametros.getBytes().length));
